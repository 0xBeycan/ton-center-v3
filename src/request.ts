...
export interface BlocksRequest extends Omit<GeneralRequest, 'account' | 'tx_hash' | 'msg_hash'> {
    workchain?: number
    shard?: string
    seqno?: number
}

export interface MessagesRequest extends Omit<GeneralRequest, 'account' | 'tx_hash' | 'mc_seqno'> {
    msg_hash?: string[]
    body_hash?: string
    source?: string
    destination?: string
